...
- [Supabase Tutorial](#supabase-tutorial)
  - [01. Getting Started - Build a User Management App with SvelteKit](#01-getting-started---build-a-user-management-app-with-sveltekit)
    ...
    - [Automatic Purging Old Avatars](#automatic-purging-old-avatars)
...
Create `src/routes/account/Avatar.svelte`:
```svelte
...
Add to `src/routes/account/+page.svelte`:
```svelte
<script lang="ts">
...
### Automatic Purging Old Avatars

Currently profile photos are uploaded with a random name to the `avatars` bucket.

So new photos will be orphaned and stay in the storage until deleted.

> I don't know why this is preferable over overwriting the existing photo for the user.

You can add HTTP trigger and postgres functions to handle removal of old images.
